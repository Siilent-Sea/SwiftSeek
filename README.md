...
当前仓库不是空项目：`v1-baseline`、`everything-alignment`、`everything-performance`、`everything-footprint`、`everything-usage`、`everything-ux-parity`、`everything-productization`、`everything-menubar-agent`、`everything-filemanager-integration` 均已完成归档。最近完成的 `everything-filemanager-integration` 已把“在 Finder 中显示”扩展为可配置 Finder / QSpace / 自定义文件管理器。
...
- 当前活跃轨道：暂无；最近完成轨道 `everything-filemanager-integration` 已 `PROJECT COMPLETE`
