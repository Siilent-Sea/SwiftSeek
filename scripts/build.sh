...
echo "  不写 Info.plist、不做 codesign / notarization。"
echo ""
echo "[build.sh] 需要可启动的 .app bundle？(K2)"
echo "  ./scripts/package-app.sh                # 标准 + ad-hoc codesign"
echo "  ./scripts/package-app.sh --sandbox      # 受限沙箱"
echo "  ./scripts/package-app.sh --no-sign      # 跳过 codesign（开发迭代）"
echo "  产物：dist/SwiftSeek.app（含 Info.plist + AppIcon.icns + 真实 GitCommit/BuildDate）"
echo "  正式 Apple Developer ID 签名 / notarization 不在 K2 范围。"
