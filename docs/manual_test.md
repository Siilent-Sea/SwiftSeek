...
#### 8. 边界：何时可以接受 Dock 出现
- L1 默认 no Dock。Dock 出现 SwiftSeek 图标的唯一合法情况：用户在 L2 设置里勾选了 "在 Dock 显示" 并已重启 SwiftSeek。
- 全新 DB / 未操作过 L2 设置时仍出现 Dock = ❌

### 33z. L2 Dock 显示开关 + activation policy 切换（everything-menubar-agent）

L2 把"用户能否恢复 Dock 图标"做成一个持久化设置，重启生效。本节是 release_checklist §5c 的展开版。

#### 准备
- 干净 workspace
- §33t 打过的 `dist/SwiftSeek.app`，GitCommit 与 HEAD 一致
- 删除现有 DB 模拟全新安装：`rm -rf ~/Library/Application\ Support/SwiftSeek`（**会丢索引数据**，确认是测试机后再做；或 `HOME=/tmp/...` 跑）

#### 1. 全新 DB 默认 no Dock
1. `open dist/SwiftSeek.app`
2. 看 Dock：**没有** SwiftSeek 图标
3. 看菜单栏：放大镜图标存在
4. Console 应有日志 `Dock icon hidden (L1 default); activation policy = .accessory`
5. 设置 → 常规 → 滚到最底
6. **预期**：复选框 "在 Dock 显示 SwiftSeek 图标（菜单栏入口仍保留）" **未勾选**
7. **预期**：下方 note "✓ 当前以菜单栏 agent 形态运行（默认）：Dock 中不显示，仅保留菜单栏入口。下次启动会沿用此设置。"

#### 2. 切到 Dock visible（user opt-in）
1. 勾选复选框
2. **预期**：note 立即变成 "⚠️ 已勾选「在 Dock 显示」，但当前进程仍是菜单栏 agent 模式（`.accessory`）。请退出 SwiftSeek（菜单栏 → 退出）并重新启动后生效。"
3. **此时 Dock 仍不显示** SwiftSeek（这是 design：不做 live transition）
4. 菜单栏 → "退出 SwiftSeek"
5. `open dist/SwiftSeek.app`
6. **预期**：这次 Dock **显示** SwiftSeek 图标
7. Console 应有 `Dock icon visible (user preference); activation policy = .regular`
8. 菜单栏图标 **同时存在**（不是二选一）

#### 3. Dock visible 模式下入口验证
1. 点 Dock 图标 → 任一窗口前置（如有可见窗口）
2. 菜单栏 → "搜索…" → 搜索窗前置可输入
3. 菜单栏 → "设置…" → 设置窗前置
4. ⌥Space 全局热键 → 搜索窗呼出
5. ESC 隐藏搜索窗
6. 设置 → 常规 → 最底：复选框 **已勾选**，note "✓ 当前以普通 App 形态运行：Dock 中可见..."

#### 4. 切回 no Dock 模式
1. 取消勾选复选框
2. **预期**：note 变成 "⚠️ 已取消勾选，但当前进程仍以普通 App 形态运行（`.regular`）。请退出 SwiftSeek（菜单栏 → 退出）并重新启动后生效。"
3. **此时 Dock 仍显示** SwiftSeek（不做 live transition）
4. Dock 右键 SwiftSeek → 退出（这次因为 Dock visible 所以右键退出可用）
5. `open dist/SwiftSeek.app`
6. **预期**：Dock 这次不再显示 SwiftSeek
7. Console 回到 `Dock icon hidden (L1 default); activation policy = .accessory`

#### 5. 反复切换稳定性
1. 重复 §2 → §3 → §4 共 3 个完整循环
2. **预期**：每次都按设置切换；不出现菜单栏图标丢失、设置窗口卡死、菜单栏退出无响应
3. 每次循环后看一眼 Console，确认 `getDockIconVisible` 路径不报错

#### 6. 设置读取失败的 fallback（人为模拟）
1. 退出 SwiftSeek
2. 用 sqlite CLI 把 `settings` 表里 `dock_icon_visible` 行写成非法值（如 `"abcdef"`）：`sqlite3 ~/Library/Application\ Support/SwiftSeek/index.sqlite3 "UPDATE settings SET value='abcdef' WHERE key='dock_icon_visible'"`
3. `open dist/SwiftSeek.app`
4. **预期**：app 正常启动；Dock 不显示（fallback 到 L1 默认 no Dock）；Console 应有 `getDockIconVisible failed, keeping L1 .accessory default: ...` 或 `getDockIconVisible` 返回 false（"abcdef" != "1"）
5. 这一步证明读取失败/异常值都不会让 SwiftSeek 没有入口

#### 7. 与其他设置不互相干扰
1. 在两种模式下分别测试：
   - 切换 "索引隐藏文件" 复选框：仍能正常持久化 / 立即重建按钮可点
   - 切换 "随 macOS 登录自动启动 SwiftSeek"：仍能正常调 SMAppService
   - 设置搜索结果上限 / 切换索引模式 / 切换全局热键：都不受 Dock 模式影响
2. 这一步证明 L2 没有引入意外 side effect
...
### 33ac. M3 Reveal Target 动态文案 / fallback / 诊断（everything-filemanager-integration）

M3 把 M2 的运行时路由真正暴露到用户可见层：搜索窗口按钮 / 右键菜单 / fallback toast / Diagnostics 都跟随当前 reveal target 变化。

#### 准备
- 干净 workspace；§33t 打过的 `dist/SwiftSeek.app`
- 启动 SwiftSeek，菜单栏图标可见；ESC 关闭搜索窗

#### 1. Finder 模式（默认）
1. 设置 → 常规 → "显示位置" 应是 Finder（默认值）
2. ⌥Space 唤出搜索窗 → 输入任意字符直到出结果 → 点击一行
3. **预期按钮**：底部按钮显示 `在 Finder 中显示`
4. **预期菜单**：右键结果行 → 出现 `在 Finder 中显示`
5. 点按钮 → Finder 弹出并选中目标文件
6. ESC 隐藏 → 再 ⌥Space → 文案仍是 `在 Finder 中显示`

#### 2. 切到 QSpace
1. 设置 → 常规 → "显示位置" → 自定义 App… → "选择 App…" → 选 `/Applications/QSpace.app`
2. 复选区显示 `已选 App：QSpace（已识别）` + 完整路径
3. ⌥Space 唤出搜索窗（这是 M3 关键路径：refreshRevealLabels 在 show() 里跑）
4. **预期按钮**：`在 QSpace 中显示`
5. **预期右键菜单**：`在 QSpace 中显示`
6. 点按钮 → QSpace 前置（显示父目录或文件本身，取决于 open mode）

#### 3. 切到 Path Finder（或任意 .app）
1. 设置 → 常规 → 重选 `/Applications/Path Finder.app`
2. 复选区显示 `已选 App：Path Finder.app` + 路径（不再标 "QSpace（已识别）"）
3. ⌥Space → 搜索窗
4. **预期按钮**：`在 Path Finder 中显示`（去掉了 `.app` 后缀）
5. **预期右键菜单**：`在 Path Finder 中显示`

#### 4. fallback toast 文案
1. 设置中保留 QSpace.app 路径
2. **手动制造失效**：`mv /Applications/QSpace.app /Applications/QSpace.bak.app`
3. ⌥Space → 搜索 → 点 `在 QSpace 中显示`
4. **预期 toast**：`⚠️ 无法用 QSpace 显示，已回退到 Finder：自定义 App 不存在：/Applications/QSpace.app`（前缀含 ⚠️，含 "QSpace"，含 "回退到 Finder"）
5. Finder 弹出并选中**原始文件**（不是父目录）
6. 还原：`mv /Applications/QSpace.bak.app /Applications/QSpace.app`

#### 5. Diagnostics / About 复制
1. 设置中切到 QSpace + 文件本身（item）模式
2. 设置 → 关于 → "复制诊断信息"
3. 粘贴到文本编辑器，搜 `Reveal target（M3）：`
4. **预期块**：
   ```
   Reveal target（M3）：自定义 App
     显示名称：QSpace
     按钮文案：在 QSpace 中显示
     打开模式：文件本身（item）
     自定义 App 路径：/Applications/QSpace.app
   ```
5. 切回 Finder + parentFolder → 重新复制 → 块变成：
   ```
   Reveal target（M3）：Finder（默认）
     显示名称：Finder
     按钮文案：在 Finder 中显示
     打开模式：父目录（parentFolder）
     自定义 App 路径：—（Finder 模式）
   ```

#### 6. Run Count 不变
1. 记下某文件当前 Run Count（结果行 "打开次数" 列）
2. 用任意 reveal 模式（Finder / QSpace / fallback）多次点 reveal 按钮
3. 重新搜索同一文件
4. **预期**：Run Count 数值不变；reveal 不计入 Run Count（只有 Enter / 双击的 `.open` 才会增加）
5. 然后 Enter 打开一次 → 再搜 → Run Count +1，证明 .open 仍在工作

#### 7. 边界：明确不实现 / 不承诺
- 外部 app 是否能"选中"具体文件由该 app 自身实现决定；M3 只保证 reveal 把目标 URL（item 或 parentFolder）通过 `NSWorkspace.open(...:withApplicationAt:configuration:)` 公开 API 交给该 app
