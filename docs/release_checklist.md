...
**当前 release gate** = K6 收口 + L1-L4 菜单栏 agent + M1-M4 文件管理器集成 + N1-N4 dockless hardening。未签名 / 未公证 / 无 DMG / 无 auto updater 是当前轨道明确边界。N1 暴露 Dock 状态诊断；N2 默认包写 `LSUIElement=true`（agent），`--dock-app` 写 `false`；N3 设置页显示 Dock 详情 + "恢复菜单栏模式" 一键自救按钮；N4 把 fresh DB / `dock_icon_visible=1` / `dock_icon_visible=0` / 默认包 / `--dock-app` 包 / stale bundle / 菜单栏热键 全部纳入 release gate（§5g）。L1 起 SwiftSeek 默认**不显示 Dock 图标**；菜单栏状态项是主入口；L4 起重复打开同一 bundle id 的实例会主动 defer + 唤醒旧实例；M1-M3 起"在 Finder 中显示"可配置为任意 `.app`。
...
- [ ] 当前基准 N = **262**（everything-dockless-hardening N1 落地后），本轮总数不应少于 262
...
## 5g. N1-N4 Dockless hardening 硬 gate（每次发布必跑）

`everything-dockless-hardening` 把 Dock 是否隐藏从历史 L1/L2 文档声明升级为真实 `.app` release gate。**N4 起以下组合都必须人工实测过才允许发布**。

### 自动化前置（与 §1-§3 共享，记录在此供追溯）
- [ ] `swift run --disable-sandbox SwiftSeekSmokeTest` ≥ **270**（N3 落地后基线；N1+N3 共贡献 14 cases）
- [ ] 默认 `./scripts/package-app.sh --sandbox`：banner 显示 `mode: agent (no-Dock / menu bar agent (default))` + `LSUIElement: true`，断言行 `LSUIElement assertion OK (=true, mode=agent)`
- [ ] `./scripts/package-app.sh --sandbox --dock-app`：banner 显示 `mode: dock_app (...)` + `LSUIElement: false`，断言行 `LSUIElement assertion OK (=false, mode=dock_app)`
- [ ] `./scripts/package-app.sh --help` 列出 `--dock-app` / `--no-dock` / `--agent` / `--sandbox` / `--no-sign`

### Scenario A：Fresh DB + 默认 agent 包
1. `rm -rf "$HOME/Library/Application Support/SwiftSeek"`（**注意**：会清掉本机 SwiftSeek 索引；仅在 release QA 机器上做）
2. `./scripts/package-app.sh` → `open dist/SwiftSeek.app`

- [ ] Dock 中**没有** SwiftSeek 图标
- [ ] 菜单栏放大镜图标可见
- [ ] Console 过滤 SwiftSeek，看到 `Dock — Info.plist LSUIElement=true; persisted dock_icon_visible=0; chosen activation policy=.accessory`
- [ ] **不**出现 `Dock visible because user setting dock_icon_visible=1` 行
- [ ] 设置 → 关于 → 复制诊断信息 → `Dock 状态（N1）：` 块显示 `intended mode：隐藏 Dock` + `effective activation policy：accessory` + `Info.plist LSUIElement：true`
- [ ] 设置 → 常规 → Dock 详情块显示 `✓ 当前以菜单栏 agent 形态运行（默认）`，无 ⚠️ 警告，"恢复菜单栏模式" 按钮**隐藏**

### Scenario B：Existing DB with `dock_icon_visible=1`（旧设置污染）
1. 接 Scenario A，关闭 SwiftSeek
...
3. 重新 `open dist/SwiftSeek.app`（仍是默认 agent 包）

- [ ] Dock 中**出现** SwiftSeek 图标（即使 `LSUIElement=true`，runtime activation policy 是真正控制源）
- [ ] Console 出现 `Dock — Info.plist LSUIElement=true; persisted dock_icon_visible=1; chosen activation policy=.regular` + `Dock visible because user setting dock_icon_visible=1; toggle off in Settings → ...`
- [ ] 设置 → 常规 → Dock 详情块显示 `用户意图：用户希望显示 Dock` + `effective activation policy：regular（Dock 可见）` + `Info.plist LSUIElement：true（包体声明 agent）`
- [ ] "恢复菜单栏模式" 按钮**可见**

### Scenario C：N3 一键恢复 → fresh restart
1. 接 Scenario B
2. 设置 → 常规 → 点 "恢复菜单栏模式（隐藏 Dock）" → 确认 NSAlert
3. 菜单栏 → "退出 SwiftSeek" → `open dist/SwiftSeek.app`

- [ ] 重启后 Dock 中再无 SwiftSeek 图标
- [ ] sqlite3 verify：`SELECT value FROM settings WHERE key='dock_icon_visible';` 返回 `0`
- [ ] Console 重启日志符合 Scenario A 预期
- [ ] 没有任何索引数据 / 其他设置被改动（diagnostics 中其他字段不变）

### Scenario D：`--dock-app` 包
1. `./scripts/package-app.sh --dock-app`
2. `cp -R dist/SwiftSeek.app /tmp/SwiftSeek-dockapp.app && open /tmp/SwiftSeek-dockapp.app`（用 `/tmp` 避免污染主 install）

- [ ] Dock 中出现 SwiftSeek 图标
- [ ] 菜单栏图标也存在
- [ ] Console 显示 `Dock — Info.plist LSUIElement=false; ...`
- [ ] 设置 → 常规 → Dock 详情块显示 `Info.plist LSUIElement：false（包体允许 Dock）`
- [ ] 关闭后回默认 agent 包，Dock 仍按用户设置决定

### Scenario E：Stale bundle / wrong bundle path
1. 同时存在 `/Applications/SwiftSeek.app`（旧 commit）和 `dist/SwiftSeek.app`（当前 commit）
2. `open dist/SwiftSeek.app`

- [ ] Console 启动三连 `bundle=/Users/.../dist/SwiftSeek.app` 与你的预期一致（不是 `/Applications/...`）
- [ ] 设置 → 关于 → Diagnostics 的 `Dock 状态（N1）：` 块 + Settings detail 块都显示 `bundle path` 与上面一致
- [ ] 切换菜单栏 → 退出 → `open /Applications/SwiftSeek.app` → bundle path 反过来；用户能据此立刻识别 stale bundle

### Scenario F：菜单栏 + 热键不回归
- [ ] 默认 agent 包：菜单栏 → 搜索…（⌥Space）打开搜索窗
- [ ] 菜单栏 → 设置… 打开设置窗
- [ ] 菜单栏 → 退出 SwiftSeek 干净退出
- [ ] 全局热键独立可呼出搜索窗（不依赖菜单栏）
- [ ] 上述所有项在 `--dock-app` 包下也通过
