...
- K5 实现已就位（待 Codex 验收）：
  - `Sources/SwiftSeekCore/SettingsTypes.swift`：`RootHealth` 拆出 `.volumeOffline`，新增 `RootHealthReport` 结构 + `Database.computeRootHealthReport(for:currentlyIndexingPath:)`。`/Volumes/<X>/...` 路径在 `<X>` 卷未挂载时返回 `.volumeOffline`，否则真实缺路径返回 `.offline`，权限被拒返回 `.unavailable`，可读返回 `.ready`。
  - `Sources/SwiftSeekCore/Diagnostics.swift`：诊断块新增 `roots 健康（K5）：` 段，最多列 20 个 root，每行 `<徽标>  <路径>  — <detail>`，与 UI 同源。
  - `Sources/SwiftSeek/UI/SettingsWindowController.swift`：rootsTable cell 改用 `RootHealthReport`，detail 挂到 NSTextField.toolTip；rootsBar 新增 "重新检查权限"（reload UI 不动 DB）+ "打开完全磁盘访问设置"（`x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles` deep-link，失败回退通用隐私 + NSAlert）。rootsStatus 文案改用 K5 状态词汇。
  - `Sources/SwiftSeekSmokeTest/main.swift`：增加 6 个 K5 用例（ready / offline / volumeOffline / unavailable / paused 覆盖 / Diagnostics 同源），E4 uiLabel 关键字同步更新；当前 SmokeTest 209/209。
  - `docs/install.md`：新增"权限 / Full Disk Access / Root 覆盖（K5）"段，列四种状态徽标含义 + FDA 引导 + recheck 流程。
  - `docs/known_issues.md` §8 改写为 K5 已落地。
  - `docs/manual_test.md` 新增 §33w：四种状态矩阵 + recheck 路径 + FDA 跳转 fallback + 诊断同源校验。
  - 受限沙箱下 `swift build --disable-sandbox` + `swift run --disable-sandbox SwiftSeekSmokeTest` 209/209 通过。
